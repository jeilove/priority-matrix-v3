...
async function testRaw() {
  console.log('🔗 원시 DB 연결 테스트 중...');
  console.log('주소:', connectionString ? connectionString.substring(0, 30) + '...' : '누락됨');
  ...
  try {
    ...
    const recentTodos = await client.query('SELECT text, "userId" FROM "Todo" ORDER BY "createdAt" DESC LIMIT 5');
    console.log('\n--- 최근 할일 5개 ---');
